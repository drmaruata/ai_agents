...
class InMemoryRepository(Repository):
    """Deterministic repository used by tests and local development without Postgres."""
...
class PostgresRepository(Repository):
    ...
    def create_task(self, task: Task) -> Task:
        sql = """
        INSERT INTO tasks (
            task_id, project_id, title, description, risk_level, assigned_agent,
            status, dependencies, allowed_paths, acceptance_criteria, metadata,
            version, created_at, updated_at
        ) VALUES (
            %(task_id)s, %(project_id)s, %(title)s, %(description)s, %(risk_level)s,
            %(assigned_agent)s, %(status)s, %(dependencies)s, %(allowed_paths)s,
            %(acceptance_criteria)s, %(metadata)s, %(version)s, %(created_at)s, %(updated_at)s
        ) RETURNING *
        """
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(sql, _task_params(task))
                row = cur.fetchone()
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO task_runs (
                        run_id, task_id, agent, status, attempt, started_at, finished_at, error, output_artifact_ids
                    ...
                    """,
                    (
                        run.run_id, run.task_id, run.agent.value, run.status.value, run.attempt,
                        run.started_at, run.finished_at, run.error, run.output_artifact_ids,
                    ),
                )
                row = cur.fetchone()
    ...
    def audit(self, event: AuditEvent) -> None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO audit_events (event_id, event_type, actor, task_id, device_id, metadata, created_at)
                    VALUES (%s,%s,%s,%s,%s,%s,%s)
                    """,
                    (
                        event.event_id, event.event_type.value, event.actor, event.task_id,
                        event.device_id, event.metadata, event.created_at,
                    ),
                )
...
def _row_to_task(row: dict[str, Any]) -> Task:
    return Task(
        ...
        description=row["description"], risk_level=row["risk_level"],
        assigned_agent=row["assigned_agent"], status=row["status"],
...
def _row_to_run(row: dict[str, Any]) -> TaskRun:
    return TaskRun(
        run_id=row["run_id"], task_id=row["task_id"], agent=row["agent"],
        status=row["status"], attempt=row["attempt"], started_at=row["started_at"],
        finished_at=row["finished_at"], error=row["error"], output_artifact_ids=row["output_artifact_ids"] or [],
    )
